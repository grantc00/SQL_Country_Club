...
SELECT sub.facility, sub.first_name || ' ' || sub.last_name AS fullname, sub.cost
FROM (
    SELECT Facilities.name AS facility, Members.firstname AS first_name, Members.surname AS last_name,
...
FROM (SELECT Facilities.name AS facility,
      ...
      FROM Bookings JOIN Members USING(memid) JOIN Facilities USING(facid)) AS total_cost_per_booking
...
SELECT f.name, SUM(slots) AS usage_by_member
FROM Facilities f
...
SELECT F.name, strftime('%m', starttime) AS month, SUM(slots) AS usage
